...
New component: [`web/src/components/SearchModeToggle.vue`](../web/src/components/SearchModeToggle.vue).
...
For `scripts/enrich_vibe.py` on your machine or CI — **not** for the Worker, **not** in the SPA:
...
python3 scripts/enrich_vibe.py --force

# 3. NEW — pack static vibe index
python3 scripts/build_vibe_index.py
...
```bash
python3 scripts/enrich_vibe.py [--sample PATH] [--force] [--limit N] [--ids 1,2,3]
```

**Algorithm:**

1. Load `CLOUDFLARE_ACCOUNT_ID` and `CLOUDFLARE_API_TOKEN` from environment or `.env.deploy`.
2. For each `sample-data/tags/{id}/metadata.json`:
   ...
   - Write sidecar: `sample-data/vibe/embeddings/{id}.json` → `{ "vector": number[1024] }`
3. Batch with exponential backoff on 429/5xx.
4. Write `sample-data/vibe/manifest.json`:
...
### `scripts/build_vibe_index.py` (to implement)

**Inputs:** `sample-data/vibe/embeddings/*.json`, metadata sentiment fields.
...
### Changes to `scripts/build_indexes.py`
...
Add to [`.env.deploy.example`](../.env.deploy.example) when implementing.
...
| `scripts/enrich_vibe.py` | To create | Offline sentiment + embeddings |
| `scripts/build_vibe_index.py` | To create | Pack `vibe.bin` + `vibe.json` |
| `scripts/build_indexes.py` | To change | Project sentiment fields |
...
| `.env.deploy.example` | To change | CF token + vibe env vars |
...
5. **Docs + deploy** — update PUBLISH.md, `.env.deploy.example`; S3 deploy with indexes.
