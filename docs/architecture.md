...
- Stores: catalog, favorites, queue, practice, recent (IndexedDB via fake-indexeddb)
- Composables: `useOnline`, `useObjectUrls`, `useTagDetail`
- Component/view smoke: SearchChips, SheetViewer, FilterSheet, TagPlayer, App, Home/Tag/Favorites/Queue/PitchPipe
...
- **Search by Vibe** (planned): Cloudflare Worker + offline embeddings — see [vibe-search](plans/vibe-search.md)
...
- Full ~7.1k media publish still ops work ([publish.md](publish.md))
...
- PWA: uncached learning-track media still needs network unless the tag was favorited with media