...
Retired remux path: `sample-data/` + `build/_retired/seed_sample.py` (removed). Use `library/` + `build/build_indexes.py`. Offline pack formats: [decisions/offline-library.md](decisions/offline-library.md). Audio tiers: [decisions/audio-storage-cache.md](decisions/audio-storage-cache.md) and `sync/docs/AUDIO_STORAGE_AND_CACHE.md`.
