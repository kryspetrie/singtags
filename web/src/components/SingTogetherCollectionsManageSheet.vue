...
/**
 * Manage Sing Together repertoire collections: reorder, rename, delete, and create.
 * Modeled on CollectionsManageSheet (Favorites).
 */
...
const pendingDeleteMessage = computed(() => {
  const col = pendingDeleteId.value ? store.collectionById(pendingDeleteId.value) : null
  if (!col) return ''
  return `Delete “${col.name}”? Songs stay in your repertoire — only this collection is removed.`
})

const pendingRenameTitle = computed(() => {
  const col = pendingRenameId.value ? store.collectionById(pendingRenameId.value) : null
...
function confirmCreateDialog(): void {
  createError.value = null
  const nameError = store.validateCollectionName(createInput.value)
  if (nameError) {
    createError.value = nameError
    return
  }
  const col = store.createCollection(createInput.value)
  if (!col) {
...
function openRenameDialog(id: string): void {
  if (!store.collectionById(id)) return
...
function confirmRenameDialog(): void {
  ...
  if (!id) return
  renameError.value = null
  const nameError = store.validateCollectionName(renameInput.value, id)
  if (nameError) {
    renameError.value = nameError
    return
  }
  if (!store.renameCollection(id, renameInput.value)) {
...
<template>
  <FilterSheet :open="open" title="Manage collections" full-screen elevated @close="emit('close')">
    <div class="manage">
      <p class="manage-desc">
        Collections group songs in your repertoire on this device. Drag the ⠿ handle to reorder —
        collections at the top appear first here and in pickers. Removing a song from your
        repertoire also removes it from every collection.
      </p>
      <ul
        ...
        >
          ...
          <div class="manage-main">
            <div class="manage-title-row">
              ...
              <span class="manage-count">
                {{ c.songIds.length }} song{{ c.songIds.length === 1 ? '' : 's' }}
              </span>
            </div>
          ...
          <div class="manage-row-actions">
            ...
            <button
              ...
              @click="requestDeleteCollection(c.id)"
            >
              Delete
            </button>
          </div>
