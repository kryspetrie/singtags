...
describe('snackbar store', () => {
  ...
  it('error replaces a visible ok immediately', () => {
    const snack = useSnackbarStore()
    snack.show('Saved', { tone: 'ok', ms: 0 })
    snack.show('Save failed', { tone: 'error', ms: 0 })
    expect(snack.message).toBe('Save failed')
    expect(snack.tone).toBe('error')
  })

  it('queues ok while error is showing, then plays after dismiss', () => {
    const snack = useSnackbarStore()
    snack.show('Boom', { tone: 'error', ms: 0 })
    snack.show('Also ok', { tone: 'ok', ms: 0 })
    expect(snack.message).toBe('Boom')
    expect(snack.tone).toBe('error')
    snack.dismiss()
    expect(snack.message).toBe('Also ok')
    expect(snack.tone).toBe('ok')
  })

  it('new error clears queued non-errors', () => {
    const snack = useSnackbarStore()
    snack.show('First error', { tone: 'error', ms: 0 })
    snack.show('Stale ok', { tone: 'ok', ms: 0 })
    snack.show('Second error', { tone: 'error', ms: 0 })
    expect(snack.message).toBe('Second error')
    snack.dismiss()
    expect(snack.message).toBeNull()
  })

  it('caps pending non-errors at 3 (drops oldest)', () => {
    const snack = useSnackbarStore()
    snack.show('Error', { tone: 'error', ms: 0 })
    snack.show('a', { tone: 'info', ms: 0 })
    snack.show('b', { tone: 'info', ms: 0 })
    snack.show('c', { tone: 'info', ms: 0 })
    snack.show('d', { tone: 'info', ms: 0 })
    snack.dismiss()
    expect(snack.message).toBe('b')
    snack.dismiss()
    expect(snack.message).toBe('c')
    snack.dismiss()
    expect(snack.message).toBe('d')
    snack.dismiss()
    expect(snack.message).toBeNull()
  })
})
