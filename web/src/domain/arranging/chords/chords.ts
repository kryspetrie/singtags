 ...
 * Canonical home for arranging + SingTags Tag Studio (re-export shim).
 * Inspired by MuseScore Barbershop Harmonizer; keep chord IDs / voicing strings stable.
 */
...
/** Voicing strings: bass | bari | lead | tenor chord-tone roles. */
...
/** Dom9 omit strategy from bass role in the voicing string. */
export type Dom9Omit = 'omit5' | 'omitRoot' | 'unknown'

export function dom9OmitStrategy(voicing: string): Dom9Omit {
  if (voicing.length < 1) return 'unknown'
  const bass = Number(voicing[0])
  if (bass === 1) return 'omit5'
  if (bass === 5) return 'omitRoot'
  return 'unknown'
}
